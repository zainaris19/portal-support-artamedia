...
from notifications import notify_ticket_event  # noqa: E402
...
def build_crm_helpdesk_router(get_current_user, get_db):
    ...
    async def reassign_ticket(tid: str, body: ReassignIn, user: dict = Depends(get_current_user)):
        ...
        t = await _get_ticket_or_404(db, tid)
        if t["status"] == "SELESAI":
            raise HTTPException(status_code=400, detail="Ticket sudah SELESAI, tidak dapat dialihkan")
        entry = {
            "id": new_id(),
            "at": now_iso(),
            "from_id": t.get("troubleshooter_id"),
            "from_name": t.get("troubleshooter_name"),
            "to_id": body.new_troubleshooter_id,
            "to_name": body.new_troubleshooter_name,
            "reason": body.reason,
            "by_id": user.get("id"),
        ...
        await db.helpdesk_tickets.update_one(
            {"id": tid},
            {
                ...
                "$set": {
                    "troubleshooter_id": body.new_troubleshooter_id,
                    "troubleshooter_name": body.new_troubleshooter_name,
                    "updated_at": now_iso(),
                },
            },
        )
    ...
    async def list_technicians(user: dict = Depends(get_current_user)):
        ...
        docs = await db.users.find(
            {"role": {"$in": ["admin", "supervisor", "engineer"]}, "active": {"$ne": False}},
