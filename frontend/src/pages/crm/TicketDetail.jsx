...
export default function TicketDetail() {
  ...
  useEffect(() => { if (isSupervisor) api.get('/crm/technicians').then(({ data }) => setUsers(data || [])).catch(() => {}); }, [isSupervisor]);
...
function Timeline({ ticket }) {
  ...
  (ticket.reassign_history || []).forEach((r) => {
    events.push({ at: r.at, title: 'Troubleshooter dialihkan', who: r.by_name, icon: Users, color: 'violet', note: `${r.from_name || '—'} → ${r.to_name}` });
  });
        ...
        <div className="mt-4 space-y-3">
          ...
          <F label="Troubleshooter Baru *">
            <Select value={uid} onValueChange={setUid}>
              ...
              <SelectContent>
                {users.filter((u) => u.active !== false && ['admin', 'supervisor', 'engineer'].includes(u.role)).map((u) => (
