...
export default function NotificationGateway() {
  ...
  const [form, setForm] = useState({
    ...
    sender: '', default_group: '', main_group: '', send_closing_resume: false, country_code: '62', enabled: false, public_base_url: '',
  ...
  const load = async () => {
    ...
    try {
      ...
      setForm((f) => ({
        ...
        send_closing_resume: !!data.send_closing_resume,
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        default_group: form.default_group, main_group: form.main_group,
        send_closing_resume: form.send_closing_resume, country_code: form.country_code,
          ...
          {!isEmail && (
            <div className="space-y-5 rounded-lg border border-border p-4" data-testid="notif-wa-fields">
              ...
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  <Label>Main / Management Group (ID grup WhatsApp)</Label>
                  <Input data-testid="wa-maingroup-input" value={form.main_group} onChange={(e) => set('main_group', e.target.value)} placeholder="120363xxxxxxxxxxxx@g.us" />
                </div>
              </div>
              <div className="flex items-center justify-between rounded-lg border border-border p-3 bg-muted/20">
                <div className="pr-4">
                  <div className="text-sm font-medium">Kirim Resume Penyelesaian ke Main Group</div>
                  <div className="text-xs text-muted-foreground">Saat ticket berstatus SELESAI, kirim 1 resume ringkas ke Main/Management Group. Grup operasional tetap menerima notifikasi seperti biasa.</div>
                </div>
                <Switch data-testid="wa-closing-resume-switch" checked={form.send_closing_resume} onCheckedChange={(v) => set('send_closing_resume', v)} />
              </div>
            </div>
          )}
